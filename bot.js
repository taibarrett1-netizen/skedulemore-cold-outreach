  ...
  for (;;) {
    ...
    try {
      ...
      if (!sendResult.ok && sendResult.reason === 'hourly_limit') {
        ...
        //
        // Old behavior: forceRotate proxy assignment and update cold_dm_instagram_sessions.proxy_url.
        // New behavior: relaunch Chrome and retry using the same proxy. If it remains down, pause.
        const allowRotate =
          process.env.COLD_DM_ALLOW_PROXY_ROTATION_ON_TUNNEL_FAILURE === '1' ||
          process.env.COLD_DM_ALLOW_PROXY_ROTATION_ON_TUNNEL_FAILURE === 'true';
        ...
        if (
          ...
          try {
            ...
            if (rotatedProxy?.proxyUrl) {
              ...
              logger.warn(
                `[send-worker] Proxy tunnel failed for sender @${session.instagram_username}; rotated proxy assignment (COLD_DM_ALLOW_PROXY_ROTATION_ON_TUNNEL_FAILURE=1) and will retry.`
              );
              // Done (rotated and scheduled retry).
              handledSendCooldownInline = true;
            }
          } catch (proxyRotateErr) {
            logger.warn(
              `[send-worker] Proxy rotation after tunnel failure failed for sender @${session?.instagram_username || 'unknown'}: ${
        ...
        if (!handledSendCooldownInline) {
          // Default: pause instead of swapping exit IPs under the same session cookies.
